...
**重要 — 每個 mainFlow / exceptionFlow 步驟必須是「角色 → 操作 → 系統反應」三段式**，例如：
- ✅ "工地主任在「任務指派」頁面，於「施工項目」下拉選單選擇項目 → 系統從「施工項目模組」拉取對應人機料清單"
- ❌ "選擇施工項目"（缺角色、UI 位置、後台行為）
...
**⚠️ 重要**：
- **以「對話歷史」為主要事實來源**——SA 在對話中提到的所有具體流程、判斷條件、欄位校驗、班別累加、編輯機制、Audit Log 等細節都要反映到流程圖
...
範例（注意要包含具體業務情境，不是通用 CRUD）：

===MMD_START===
flowchart TD
  Start([現場人員開始填日報]) --> A[選擇專案 / 班別 / 區域]
  A --> B{是否屬於該專案配置人員？}
  B -->|否| Reject([無權限，拒絕])
  B -->|是| C[填寫氣候 / 施工人數 / 工料]
  C --> D{基本欄位是否填齊？}
...
- **系統管理員**：維護系統基本資料、審查報表、權限控管
- **工地主任**：現場人機料調度、排班、批次提交資料
- **現場工人**：查看每日任務、回報工作進度、出勤簽到退
\`\`\`
...
✅ 範例：
> 1. **工地主任** 在「任務指派」頁面，從「施工項目」下拉選單選擇一個項目 → 系統從「施工項目模組」拉取對應的人員/機具/物料清單顯示於下方表格
> 2. **工地主任** 在每列勾選或修改數量後，點「加入批次」按鈕 → 系統將該筆暫存到前端購物車狀態（尚未送出後台）
> 3. **工地主任** 點「批次發布」按鈕 → 系統將購物車所有資料 POST 到「任務模組」，並觸發通知服務推播給對應現場工人
